  ...
  <div className="min-h-screen bg-[#0F172A] flex items-center justify-center p-6">
    <div className="max-w-md w-full bg-slate-900/50 backdrop-blur-xl border border-slate-800 p-8 rounded-[2rem] text-center shadow-2xl animate-in fade-in zoom-in duration-500">
      ...
      <div className="space-y-3">
        <Link
          to="/"
          className="w-full py-4 bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded-2xl font-bold hover:bg-emerald-500/20 transition-all text-sm flex items-center justify-center gap-2 active:scale-[0.98]"
        >
    ...
    <BrowserRouter>
      <Routes>
        <Route 
          ...
          element={
            firebaseUser ? (
              isUserAdmin ? <AdminPage /> : <AccessDenied email={firebaseUser.email} logout={logout} />
            ) : (
              <LoginScreen />
            )
          } 
