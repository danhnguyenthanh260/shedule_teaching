...
export const StatusAlerts: React.FC<StatusAlertsProps> = ({ error, result, onClose }) => {
  ...
  return (
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[1000] w-full max-w-2xl px-6 animate-in slide-in-from-bottom-8 duration-500 ease-out">
      ...
      {result && (
        <div className="bg-white/95 backdrop-blur-xl border border-emerald-100 p-6 rounded-[2.5rem] flex flex-col sm:flex-row items-center justify-between shadow-[0_20px_50px_rgba(16,185,129,0.15)] animate-in fade-in zoom-in-95 duration-500 border-b-4 border-b-emerald-100/50">
          <div className="flex items-center gap-5 w-full sm:w-auto mb-4 sm:mb-0">
            <div className="w-14 h-14 bg-emerald-50 rounded-[1.25rem] flex items-center justify-center text-emerald-500 shadow-inner">
              <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <div>
              <h4 className="text-[10px] font-bold text-emerald-500 uppercase tracking-[0.2em] mb-1.5 flex items-center gap-2">
                <span className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse shadow-[0_0_8px_rgba(52,211,153,0.5)]" />
                {result.type === 'sync' ? 'Đồng bộ hoàn tất' : 'Dọn dẹp hoàn tất'}
              </h4>
              
              {result.type === 'sync' ? (
                <div className="flex flex-wrap gap-3">
                   <div className="flex flex-col">
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Thêm mới</span>
                      <span className="text-sm font-extrabold text-slate-800">{result.created}</span>
                   </div>
                   <div className="w-px h-6 bg-slate-100 self-end" />
                   <div className="flex flex-col">
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Bỏ qua</span>
                      <span className="text-sm font-extrabold text-amber-500">{result.skipped}</span>
                   </div>
                   <div className="w-px h-6 bg-slate-100 self-end" />
                   <div className="flex flex-col">
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Thải loại</span>
                      <span className="text-sm font-extrabold text-rose-500">{result.failed}</span>
                   </div>
                </div>
              ) : (
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto border-t sm:border-t-0 sm:border-l border-slate-100 pt-4 sm:pt-0 sm:pl-6">
            <a 
              ...
              target="_blank" 
              rel="noreferrer" 
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-[#F27024] hover:bg-orange-600 text-white px-6 py-3.5 rounded-2xl font-bold text-[10px] transition-all shadow-lg shadow-orange-100 active:scale-95 hover:-translate-y-0.5 uppercase tracking-widest whitespace-nowrap"
            >
              Xem Lịch
