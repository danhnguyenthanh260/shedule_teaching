...
/**
 * ------------------------------------------------------------------
 * GENERIC MERGED CELL FILLING (Matrix-based)
 * Supports Google Sheets 2D array structure
 * ------------------------------------------------------------------
 */

export interface GenericMergeRange {
  startRow: number;
  endRow: number;
  startCol: number;
  endCol: number;
...
/**
 * Điền giá trị vào tất cả các ô trong vùng gộp (Merged Cells).
 * Hữu ích cho việc chuẩn hóa dữ liệu trước khi xử lý.
 * @param matrix 2D array of values (rows x cols)
 * @param merges List of merge ranges
 */
export function fillMergedValues(matrix: any[][], merges: GenericMergeRange[]): any[][] {
  // Clone matrix to avoid mutation
  const filledMatrix = matrix.map(row => [...(row || [])]);

  merges.forEach(merge => {
    // Validate bounds
    if (merge.startRow < 0 || merge.startCol < 0) return;

    // Get master value (top-left cell of the merge block)
    const masterRow = filledMatrix[merge.startRow];
    if (!masterRow) return;

    const masterValue = masterRow[merge.startCol];

    // Skip if master value is empty (optional behavior, but safer to keep existing)
    // if (masterValue === undefined || masterValue === null || masterValue === '') return;

    // Fill all cells in the range
    for (let r = merge.startRow; r <= merge.endRow; r++) {
      // Ensure row exists
      if (!filledMatrix[r]) filledMatrix[r] = [];
