            ...
            {semesters.map((sem) => (
                <div key={sem.id} className="group bg-white p-4 rounded-xl border border-slate-100 hover:border-orange-200 transition-all duration-300 shadow-sm hover:shadow-md relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        {sem.sheetType !== 'review' && (
                            <div className="mb-4 flex items-center justify-between p-2.5 bg-slate-50/50 rounded-xl border border-slate-100 transition-all hover:bg-slate-50">
                                <div className="flex items-center gap-2">
                                    <div className={`p-1.5 rounded-lg transition-all ${sem.notifEnabled ? 'bg-orange-100 text-[#F27024]' : 'bg-slate-100 text-slate-400'}`}>
                                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                                        </svg>
                                    </div>
                                    <span className="text-[9px] font-bold text-slate-600 uppercase tracking-wider">Thông báo mail</span>
                                </div>
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        onSetupNotifications(sem.sheetUrl, sem.tabName, sem.id, sem.notifEnabled);
                                    }}
                                    className="relative inline-flex items-center cursor-pointer scale-90"
                                >
                                    <div className={`w-8 h-4.5 rounded-full transition-all duration-300 ${sem.notifEnabled ? 'bg-[#F27024]' : 'bg-slate-300'}`}>
                                        <div className={`absolute top-0.5 left-0.5 bg-white w-3.5 h-3.5 rounded-full shadow-sm transition-all duration-300 ${sem.notifEnabled ? 'translate-x-3.5' : 'translate-x-0'}`}></div>
                                    </div>
                                </button>
                            </div>
                        )}
