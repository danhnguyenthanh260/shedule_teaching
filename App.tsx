...
import { RowNormalized, UserProfile, SyncResult } from './types';
...
const App: React.FC = () => {
  ...
  // Khởi tạo Google Token Client khi App mount
  useEffect(() => {
    const initGoogle = () => {
  ...
  const handleLoad = async () => {
    ...
    setError(null);
    try {
      const { rows: data } = await googleService.loadSheet(sheetUrl, tabName, accessToken);
      setRows(data);
