 ...
 * Version: 13.1 - ROBUST DECENTRALIZED SYNC
 * =====================================================
 */
...
const GoogleCalendarAPI = {
  ...
  getCalendarMetadata: function(accessToken, calendarId = 'primary') {
    try {
      ...
      AppLogger.info('Could not fetch calendar metadata (likely missing calendar.readonly scope)', e.message);
          ...
          try {
            ...
            if (existing) {
               const unchanged = existing.summary === ev.title && 
                                 ...
                                 Math.abs(new Date(existing.start.dateTime).getTime() - start.getTime()) < 1000;
               if (unchanged && !force) {
                 results.skipped++;
                 return;
               }
...
function doGet(e) {
  return ContentService.createTextOutput(JSON.stringify({ 
    status: CONSTANTS.SUCCESS, version: '13.1', message: 'FPT Scheduler GAS Engine V13.1 (Robust Decentralized) is ACTIVE' 
  ...
  try {
    ...
    if (payload.secret !== CONSTANTS.GAS_SECRET) {
      ...
      if (!authResult.valid || !isAuthorized_(authResult.email)) {
          throw new Error('Unauthorized Access (V13.1)');
      }
    }
    ...
    return jsonResponse_({ status: CONSTANTS.SUCCESS, version: '13.1', data: res });
    
  } catch (err) {
    AppLogger.error('POST Error', err);
    return jsonResponse_({ status: CONSTANTS.ERROR, version: '13.1', message: err.toString() });
  }
}
