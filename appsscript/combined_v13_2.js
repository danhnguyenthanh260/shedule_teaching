 ...
 * Version: 13.2 - DECENTRALIZED CLEAR SUPPORT
 * =====================================================
 */
...
function doGet(e) {
  return jsonResponse_({ 
    status: CONSTANTS.SUCCESS, version: '13.2', message: 'FPT Scheduler GAS Engine V13.2 (Decentralized Clear) is ACTIVE' 
  });
}
  ...
  try {
    ...
    if (payload.secret !== CONSTANTS.GAS_SECRET) {
      ...
      if (!authResult.valid || !isAuthorized_(authResult.email)) {
          throw new Error('Unauthorized Access (V13.2)');
      }
    }
    ...
    if (action === 'clearCalendar') {
      ...
      return jsonResponse_({ status: CONSTANTS.SUCCESS, version: '13.2', message: 'Cleared', data: res });
    }
    ...
    return jsonResponse_({ status: CONSTANTS.SUCCESS, version: '13.2', data: res });
    
  } catch (err) {
    AppLogger.error('POST Error', err);
    return jsonResponse_({ status: CONSTANTS.ERROR, version: '13.2', message: err.toString() });
  }
}
